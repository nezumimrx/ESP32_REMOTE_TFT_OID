...
int Masked_OID_X=0,Masked_OID_Y=0;//由于在故事模式，有些格子会被替换，所以在执行pwm_calculate_movement的时候代入的是MaskedOIDX
int OID_Index=0;
int temp_X[3],temp_Y[3],temp_Angle[3];
uint8_t temp_debounce=0;
boolean OID_available=false;
int off_coding_map_counter=0;
boolean off_coding_map_test_start=false;
...
void OID_Init(){
  ...
  Serial.print("uiTemp: ");Serial.println(uiTemp);
    ...
    if(OID_ReceiveData(OID_Receive)==0){
      if((OID_Receive[0]&0xB000)==0x2000){//是手写码
        ...
        if(temp_debounce>=3){
          ...
          if(((temp_X[0]>=temp_X[1]-5)&&(temp_X[0]<=temp_X[1]+5))&&((temp_Y[0]>=temp_Y[1]-5)&&(temp_Y[0]<=temp_Y[1]+5))&&((temp_X[0]>=temp_X[2]-5)&&(temp_X[0]<=temp_X[2]+5))&&((temp_Y[0]>=temp_Y[2]-5)&&(temp_Y[0]<=temp_Y[2]+5))){
              OID_X=temp_X[0];
              OID_Y=temp_Y[0];
